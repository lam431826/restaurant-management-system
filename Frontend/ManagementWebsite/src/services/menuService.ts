...
export const updateItem = (
  ...
  api
    .put<ApiResponse<MenuItem>>(`/api/menu/items/${id}`, input)
    .then((r) => r.data);
...
export const deleteItem = (id: string): Promise<void> =>
  api.del<void>(`/api/menu/items/${id}`);

export const bulkSetAvailability = (
  ids: string[],
  available: boolean,
): Promise<void> =>
...
export const listCategories = (): Promise<MenuCategory[]> =>
  api
    .get<ApiResponse<MenuCategory[]>>("/api/menu/categories")
    .then((r) => r.data);

export const createCategory = (input: CategoryInput): Promise<MenuCategory> =>
  api
    .post<ApiResponse<MenuCategory>>("/api/menu/categories", input)
    .then((r) => r.data);
...
export const updateCategory = (
  ...
  api
    .put<ApiResponse<MenuCategory>>(`/api/menu/categories/${id}`, input)
    .then((r) => r.data);
...
export const importCsv = (file: File): Promise<ImportResult> => {
  ...
  return api
    .postForm<ApiResponse<ImportResult>>("/api/menu/import", form)
    .then((r) => r.data);
};
...
export const uploadImage = (file: File): Promise<string> => {
  ...
  form.append("file", file);
  return api
    .postForm<ApiResponse<{ url: string }>>("/api/menu/images", form)
    .then((r) => r.data.url);
};
