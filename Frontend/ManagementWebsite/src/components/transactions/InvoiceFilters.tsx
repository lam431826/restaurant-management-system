  ...
  return (
    <form onSubmit={applyFilters} className="flex flex-col gap-5">
      <div>
        ...
        <p className="text-sm text-ink-subtle mt-1">Tìm theo đơn hàng và trạng thái thanh toán</p>
      </div>
      ...
      <div className="flex flex-col gap-3 border-b border-line pb-5">
        ...
        {([
          ...
          <label key={value} className="kv-radio">
            <input type="radio" name="invoice-paid" checked={paid === value} onChange={() => setPaid(value)} />
      ...
      <div className="flex flex-col gap-2">
        <button type="submit" className="kv-btn kv-btn-primary h-10 w-full">Áp dụng bộ lọc</button>
        <button type="button" className="kv-btn kv-btn-outline-neutral h-10 w-full" onClick={resetFilters}>Đặt lại</button>
      </div>
    </form>
  )
}
