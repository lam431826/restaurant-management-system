...
const Invoices = () => {
  ...
  const loadInvoices = useCallback(async (nextFilters: FilterState) => {
    ...
    try {
      setInvoices(await getInvoices({
        ...
        paid: nextFilters.paid === 'all' ? undefined : nextFilters.paid === 'paid',
      }))
    } catch (loadError) {
  ...
  const handleGenerate = async (orderId: string, promotionCode: string | null) => {
  ...
  const handleApplyDiscount = async (promotionCode: string) => {
    ...
    setSuccess(`Áp dụng khuyến mãi cho hóa đơn ${discountInvoice.id} thành công`)
    await refreshInvoices()
  }
    ...
    <div className="flex h-[calc(100vh-var(--kv-header-height))] bg-surface overflow-hidden">
      ...
      <section className="flex-1 min-w-0 flex flex-col p-5 gap-4">
        <InvoiceToolbar
          invoices={invoices}
          loading={loading}
          onGenerate={() => { setShowGenerate(true); setSuccess('') }}
        ...
        {error && (
          <div className="flex items-center justify-between gap-4 px-4 py-3 rounded-md bg-danger-50 text-danger-700 text-md" role="alert">
            <span>{error}</span>
            <button type="button" className="font-semibold hover:underline" onClick={() => void refreshInvoices()}>Thử lại</button>
          </div>
        )}
        {success && (
      ...
      {showGenerate && (
        <GenerateInvoiceModal onClose={() => setShowGenerate(false)} onSubmit={handleGenerate} />
      )}
      {discountInvoice && (
        <ApplyDiscountModal
