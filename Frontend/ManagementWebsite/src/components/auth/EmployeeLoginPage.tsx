...
interface Emp { id: number; name: string; shift: string }
const EMPLOYEES: Emp[] = [
...
const Avatar = ({ name, index }: { name: string; index: number }) => {
  ...
  return (
    <div className="rounded-full flex items-center justify-center shrink-0 text-white font-semibold text-[13px] leading-none" style={{ width: 40, height: 40, backgroundColor: AVATAR_COLORS[index % AVATAR_COLORS.length] }}>
      {initials}
    </div>
  )
}
      ...
      <div className="bg-white flex flex-col items-center justify-between px-[72px] py-8 w-[520px] shrink-0 h-screen overflow-y-auto">
        ...
        <div className="flex flex-col gap-2 items-center w-full shrink-0" ref={dropdownRef}>
          ...
          <div className="relative w-full">
            <button onClick={() => setOpen(o => !o)} className="bg-[#f5f5f5] flex items-center gap-3 px-5 py-[10px] rounded-[12px] w-full text-left">
              ...
              <svg className={`w-5 h-5 shrink-0 text-[#636566] transition-transform duration-200 ${open ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
            ...
            {open && (
              <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-[#e5e5e5] rounded-[12px] shadow-lg z-10 overflow-hidden">
                {EMPLOYEES.map((emp, i) => (
                  <button key={emp.id} onClick={() => { setSelected(emp); setOpen(false); setPin('') }} className={`flex items-center gap-3 px-5 py-[10px] w-full text-left hover:bg-[#f5f5f5] transition-colors ${emp.id === selected.id ? 'bg-[#e8f0fd]' : ''}`}>
        ...
        <div className="flex flex-col gap-6 items-center w-full shrink-0">
          <div className="flex flex-col gap-3 items-center">
            <p className="text-[14px] text-[#636566] leading-[1.5]">{pin.length > 0 ? 'Please input your PIN to validate yourself.' : 'Enter your PIN to validate yourself.'}</p>
          ...
          <div className="flex flex-col gap-5 w-full">
            {[NUMPAD.slice(0, 3), NUMPAD.slice(3, 6), NUMPAD.slice(6, 9), NUMPAD.slice(9, 12)].map((row, ri) => (
              <div key={ri} className="flex justify-between w-full">
                {row.map(key => (
                  <button key={key} onClick={() => handleKey(key)} className={`h-[54px] w-[64px] flex items-center justify-center rounded-[8px] hover:bg-[#f5f5f5] active:scale-95 transition-all ${key === '.' ? 'opacity-30 pointer-events-none' : ''}`}>
                    {key === 'del' ? <BackspaceIcon /> : <span className={`text-[28px] leading-none text-[#202325] ${key === '.' ? 'font-normal' : 'font-medium'}`}>{key}</span>}
                  </button>
                ))}
              </div>
            ))}
        ...
        <button onClick={handleStartShift} disabled={pin.length < PIN_LENGTH} className="bg-[#025cca] flex items-center justify-center h-[60px] rounded-[12px] w-full shrink-0 hover:bg-[#0250b0] transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
