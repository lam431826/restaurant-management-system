...
import type { MenuItem, Category, TableItem, OrderItem, CartItem } from "./orders/types";
import {
  toTableItem,
...
const CashierOrders = () => {
    ...
    setTables((prevTables) =>
      prevTables.map((t) => {
        ...
        if (tableOrders.length === 0) {
          return { ...t, orderId: null, occupied: false, amount: 0, items: 0, status: "AVAILABLE" };
        }
  ...
  useEffect(() => {
    if (selectedTable && selectedTable.occupied && selectedTable.orderId && !backendOrderId) {
  ...
  const handleRemoveItem = async (orderId: string, orderItemId: string) => {
    ...
    if (orderId === "cart") {
      ...
      if (itemToRemove) {
        ...
        setMenuItems((items) =>
          items.map((item) =>
            item.id === itemToRemove.menuItemId ? { ...item, qty: Math.max(0, item.qty - 1) } : item,
          ),
        );
      }
      return;
  ...
  const handleConfirmNote = (itemId: string, text: string) => {
    ...
    setCart((prevCart) => {
      const newCart = prevCart.map((i) => (i.cartItemId === itemId ? { ...i, note: text } : i));
  ...
  const handleConfirmReject = async (orderId: string, itemId: string, text: string) => {
    try {
  ...
  const handleCreateOrder = async () => {
    ...
    try {
      await createOrder(
        selectedTable.id,
        cart.map((c) => ({ menuItemId: c.menuItemId, quantity: 1, note: c.note })),
      );
      setCart([]);
  ...
  return (
    <div className="flex flex-col h-screen bg-[#f5f5f5] overflow-hidden font-sans">
      ...
      {rejectModal.open && rejectModal.itemId !== null && rejectModal.orderId !== null && (
        <AddNoteModal
          itemId={rejectModal.itemId}
          initialText=""
          onConfirm={(itemId, text) => handleConfirmReject(rejectModal.orderId!, itemId, text)}
