...
const MyProfile = () => {
  ...
  const handleSave = async () => {
    ...
    if (idNumberError) errs.idNumber = idNumberError
          ...
          <>
            ...
            <SectionCard title="Thông tin cơ bản">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 mt-4">
                ...
                <Field label="Email">
                  <input className={`${inputCls} bg-fill text-ink-muted`} type="email" value={email} readOnly disabled />
                </Field>
              </div>
            </SectionCard>

            <SectionCard title="Thông tin công việc" collapsible defaultOpen={false}>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 mt-4">
                <Field label="Ngày bắt đầu làm việc">
                  <input type="date" className={`${inputCls} bg-fill text-ink-muted`} value={startDate} readOnly disabled />
                </Field>
                <Field label="Ghi chú">
            ...
            <SectionCard title="Thông tin cá nhân" collapsible defaultOpen={false}>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 mt-4">
                <Field label="Số CMND/CCCD">
                  <input className={inputCls} inputMode="numeric" value={idNumber}
                    onChange={e => { setIdNumber(e.target.value); if (fieldErrors.idNumber) setFieldErrors(p => ({ ...p, idNumber: '' })) }} />
                  {fieldErrors.idNumber && <p className="text-sm text-danger">{fieldErrors.idNumber}</p>}
                </Field>

                <div className="flex gap-6 min-w-0">
                  <Field label="Ngày sinh" className="flex-1 min-w-0">
                    <input type="date" className={inputCls} value={birthday}
                      onChange={e => { setBirthday(e.target.value); if (fieldErrors.birthday) setFieldErrors(p => ({ ...p, birthday: '' })) }} />
                    {fieldErrors.birthday && <p className="text-sm text-danger">{fieldErrors.birthday}</p>}
                  </Field>
