...
// sourceType/sourceReferenceId at the API boundary — the backend doesn't persist a
// separate invoice recap. invoiceId is the real Invoice UUID (sourceReferenceId); the
// human-readable invoice.code is fetched on demand from GET /api/invoices/{id}.
...
const mapVoucher = (dto: VoucherResponseDto): CashFlowVoucher => ({
  ...
  sourceInvoice:
    dto.sourceType === "INVOICE_PAYMENT" && dto.sourceReferenceId
      ? {
          invoiceId: dto.sourceReferenceId,
...
export const listCategories = async (): Promise<CashFlowCategory[]> => {
  const res = await apiClient.get<{ data: CategoryResponseDto[] }>(
    "/cashbook/categories",
  );
...
export const createCategory = async (
  ...
  const res = await apiClient.post<{ data: CategoryResponseDto }>(
    "/cashbook/categories",
    payload,
  );
...
export const updateCategory = async (
  ...
  const res = await apiClient.put<{ data: CategoryResponseDto }>(
    `/cashbook/categories/${id}`,
    payload,
  );
  return mapCategory(res.data.data);
};
...
export const deleteCategory = (id: string) =>
  apiClient.delete(`/cashbook/categories/${id}`);
...
export const getSummary = async (
  params: { fund?: CashFlowMethod } = {},
): Promise<SummaryDto> => {
  const res = await apiClient.get<{ data: SummaryDto }>("/cashbook/summary", {
    params,
  });
